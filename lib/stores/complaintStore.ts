                    ...
                    (complaintData.evidenceFiles || []).map(async (cid: string) => {
                      try {
                        if (!get().evidenceCache[cid]) {
                          const fileResponse = await fetch(`https://ipfs.io/ipfs/${cid}`)
                          if (!fileResponse.ok) return cid
